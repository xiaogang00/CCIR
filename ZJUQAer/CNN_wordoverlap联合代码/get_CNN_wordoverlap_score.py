...
#sys.argv[1]为wordoverlap所分解出的问题与答案的单词文件的目录，sys.argv[2]为测试数据中的query_id的最大值加1（训练集为46173=46172+1）(包括无法分词的问题)
...
def get_delete_word(same_word_limit):
    file_read = open('/home/dcd-qa/MLT_code_test/wordoverlap/CCIR_test_word_num.txt','rb')
        ...
        for word_1 in question_line_list:
            for word_2 in answer_temp_line_list:
                if word_1 in delete_word_list or word_2 in delete_word_list:
                    ...
                    answer_temp_same_word_num += 1
        file_write.write(str(answer_temp_same_word_num))
        file_write.write('\n')
    file_write.close()
...
def get_wordoverlap_file_score(wordoverlap_file_dir,question_num,synonym_dir,wordoverlap_limit,wordoverlap_new_dir,delete_word_list):
    for index in range(question_num):
        ...
        same_word_num = 0
        wordoverlap_file_path = os.path.join(wordoverlap_file_dir,str(index))
        synonym_file_name = os.path.join(synonym_dir,str(index))
