...
#可能会出现有的问题及一部分答案已经被分好词，但是有另外的答案无法得到分词结果而跳出。此时会有该query的分词文件，但是仍然需要人工对答案打分，然后放入结果中，覆盖原来的query文件
...
def write_words_question(item,word_list,query_id):
    ...
    file_write = open('/home/dcd-qa/MLT_code_test/wordoverlap/data/question_ansers_words_without_stopwords/' + str(query_id),'ab+')
...
def write_words_answer(item,word_list,query_id):
    #本函数将问题及答案的所选的词语存入'/home/dcd-qa/MLT_code/NLPCC/data/wordoverlap/question_ansers_words_without_stopwords/'文件夹中对应的文件内
    file_write = open('/home/dcd-qa/MLT_code_test/wordoverlap/data/question_ansers_words_without_stopwords/' + str(query_id),'ab+')
    file_write.write(item + '\t')
    for word in word_list:
...
def get_question_answer_word(train_file,stop_words_list):
    ...
    for line in f_read.readlines():
        ...
        if query_word_list[0] == 'ERROR':
            cannot_segment_query_id_list.append(query_id)
            continue
        ...
        for item in passages_list:
            ...
            write_words_answer('answer',item_passage_text_temp_word_list,query_id)
    return question_num,cannot_segment_query_id_list
    
def get_score(question_num,cannot_segment_query_id_list):
    for index in range(question_num):
        if str(index) in cannot_segment_query_id_list:
            continue
        file_read = open('/home/duanxinyu/MLT/CCIR/MLT_code_test/NLPCC/wordoverlap/data/question_ansers_words_without_stopwords/' + str(question_index),'rb')
...
def main():
    ...
    stop_words_list = get_stop_words('/home/dcd-qa/MLT_code_test/chinese_stopwords.txt')
    question_num,cannot_segment_query_id_list = get_question_answer_word(train_file,stop_words_list)
    print 'question_num : ' + str(question_num)
    #get_score(question_num,cannot_segment_query_id_list)
