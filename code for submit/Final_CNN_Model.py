...
# 本程序是使用CNN来训练模型，其中对于词频大于50的词的忽略，并且将模型存储为'my_model_architecture.json',
# 'my_model_weights.h5'。进行测试的时候可以同时输出当前测试的所有样本的dcg值和理想的dcg值
# CNN模型通过两路输入，分贝是答案和问题的，经过word2vec的矩阵表示。最后两路输出两个vector，进行点积得出分数
...
def compute_score(model, length_a, length_q):
    ...
    # for i in range(1794 * 5):
    #    Total_score_dcg3.append(2.98782336879)
    #    Total_score_dcg5.append(4.11274032502)
