...
#sys.argv[1]为最终的得到存储新的wordoverlap的score值的文件夹(文件的名字为query_id，对于无法分词的question，人工将答案排好顺序，并且将自己设定的分数（更好的答案给更高的分数）写入一个以query_id命名的文件中，将这些文件与新的wordoverlap形成的分数文件都放在这个文件夹里)，各个score值对应与于最终的各个答案的顺序，sys.argv[2]为含有的question的数目(包括无法分词的question)
...
def write_get_result_file(query_id,query,answer_score_file_path,result_json_path):
    file_read = open(score_file_path,'rb'):
    score_index_num = {}
    score_index = 1
    for line in file_read.readlines():
        score_temp = int(line.strip())
