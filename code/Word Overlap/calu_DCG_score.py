...
#sys.argv[1]为所选k值，sys.argv[2]为score值文件的存储目录
...
def calu_avg_DCG(question_num,k,score_file_dir):
    DCG_list = [] #存储每个question的DCG值
    for id in range(question_num):
        answer_score_file = score_file_dir + '/' + str(id+1)
...
def main():
    k = sys.argv[1]
    score_file_dir = sys.argv[2]
    DCG_avg = calu_avg_DCG(4577,k,score_file_dir)
