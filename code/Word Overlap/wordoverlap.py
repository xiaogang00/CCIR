...
def write_words_question(item,word_list,line_num):
    ...
    file_write = open('/home/dcd-qa/MLT_code/NLPCC/data/wordoverlap/question_ansers_words_without_stopwords/' + str(line_num-1),'ab+')
...
def get_score(question_num):
    for index in range(question_num):
        ...
        file_read = open('/home/dcd-qa/MLT_code/NLPCC/data/wordoverlap/question_ansers_words_without_stopwords/' + str(question_index),'rb')
        question_line = file_read.readline()
        question_line_list = question_line.strip().split('\t')
        question_line_list.remove('question')
        answer_line_list = []
        file_write = open('/home/dcd-qa/MLT_code/NLPCC/data/wordoverlap/answer_label_score/' + str(question_index),'wb')
        for line in file_read.readlines():
